    ...
    >
      <nav className="mx-auto grid h-20 max-w-7xl grid-cols-[1fr_auto] items-center px-5 sm:px-8 lg:grid-cols-[1fr_auto_1fr] lg:gap-10 xl:px-0">
        <a
          ...
          className="flex min-w-0 items-center gap-2.5 pr-2 font-['Playfair_Display'] text-[0.92rem] font-normal leading-tight tracking-wide text-stone-950 transition-colors duration-300 hover:text-stone-700 sm:gap-3 sm:text-[1.02rem] md:gap-3.5 md:pr-3 md:text-[1.32rem] lg:pr-0"
          aria-label={content.nav.homeAria}
        >
          <img
            src={rivylLogo}
            alt=""
            className="h-7 w-auto shrink-0 object-contain mix-blend-multiply sm:h-8 md:h-9 lg:h-12"
            aria-hidden="true"
          />
          <span className="min-w-0 whitespace-normal text-balance">Rivyl Flooring Studio</span>
        </a>
